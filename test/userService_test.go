...
func TestServiceRegister(t *testing.T) {
	...
	type addBehaviour func(r *test.MockIUserRepository, user entities.User, ctx context.Context)

	testTable := []struct {
		ctx            context.Context
		incomingData   entities.User
	...
	}{
		{
			...
			-1,
			func(r *test.MockIUserRepository, user entities.User, ctx context.Context) {
	...
	testTable := []struct {
	...
	}{
		{
			...
			func(r *test.MockIUserRepository, ctx context.Context, phoneNumber string) {
				r.EXPECT().GetUserByPhone(ctx, phoneNumber).Return(entities.User{}, appErrors.ErrUserDoesntExist)
			},
		...
		},
		{
			...
			"cool token",
			func(r *test.MockIUserRepository, ctx context.Context, phoneNumber string) {
				r.EXPECT().GetUserByPhone(ctx, phoneNumber).Return(entities.User{}, nil)
			},
			func(h *test.MockIHasher, userPassword string, providedPassword string) {
